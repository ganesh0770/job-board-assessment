...
export default function Navbar({ currentView, setCurrentView, savedCount, appliedCount }: NavbarProps) {
  ...
  // High Fidelity Theme State Worker
  useEffect(() => {
  ...
  const handleShareProfile = () => {
    ...
    toast.success("Profile reference link copied securely to clipboard!");
  };

  return (
    <nav className="sticky z-50 w-full border-b border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-950/80 backdrop-blur-md transition-colors duration-200 mb-[] top-2 rounded-lg">
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2 cursor-pointer" onClick={() => setCurrentView('discover')}>
            ...
            <span className="font-black text-lg tracking-tight text-slate-900 dark:text-white hidden sm:block">
              Vector<span className="text-indigo-500">Hire</span>
            </span>
          </div>

          <div className="justify-end flex h-16 items-center gap-4">
            ...
            {/* Utilitarian Controls Right Flank */}
            <div className="flex items-center gap-3">
              
              {/* Notification Control Dropdown Block */}
              <div className="relative">
                ...
                {showNotifications && (
                  <div className="absolute right-0 mt-2 w-80 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-4 shadow-xl text-slate-800 dark:text-slate-100 animate-in fade-in slide-in-from-top-2 duration-150">
                    <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-2 mb-2">
                      <h4 className="text-xs font-bold uppercase tracking-wider text-slate-400">Live Workspace Updates</h4>
                      <span className="text-[10px] text-indigo-500 font-semibold cursor-pointer">Clear items</span>
                    </div>
                    <div className="space-y-3 text-xs max-h-60 overflow-y-auto">
                      <div className="p-2 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-100 dark:border-slate-800">
                        <p className="font-semibold">Application Registry verified</p>
                        <span className="text-[10px] text-slate-400">Stripe pipeline approved context delivery • 2m ago</span>
                      </div>
                      <div className="p-2 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-100 dark:border-slate-800">
                        <p className="font-semibold">New Match Alignment</p>
                        <span className="text-[10px] text-slate-400">Senior Staff Engineer position matches your #typescript query • 1h ago</span>
                      </div>
                    </div>
                  </div>
                )}
              ...
              <div className="relative" ref={profileRef}>
                ...
                {showProfileMenu && (
                  <div className="absolute right-0 mt-2 w-64 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-3 shadow-xl animate-in fade-in slide-in-from-top-2 duration-150 z-50">
                    ...
                    <div className="space-y-0.5 text-xs">
                      <button onClick={() => { setCurrentView('profile'); setShowProfileMenu(false); }} className="w-full text-left p-2 hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 rounded-xl flex items-center gap-2">
                        <User className="h-3.5 w-3.5" /> Workspace Profile
                      </button>
                      
                      <button onClick={handleShareProfile} className="w-full text-left p-2 hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 rounded-xl flex items-center gap-2">
                        <Share2 className="h-3.5 w-3.5" /> Share Profile Link
                      </button>

                      {/* Expandable Theme Selection Action */}
                      <div className="border-t border-b border-slate-100 dark:border-slate-800/60 my-1 py-1">
                        <button 
                          onClick={() => setShowThemeSubmenu(!showThemeSubmenu)} 
                          className="w-full text-left p-2 hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 rounded-xl flex items-center justify-between"
                        >
                      ...
                      <button className="w-full text-left p-2 hover:bg-slate-100 dark:hover:bg-slate-800 text-rose-500 rounded-xl flex items-center gap-2 mt-1">
                        <LogOut className="h-3.5 w-3.5" /> Evict Token Session
                      </button>
                    </div>
                  </div>
                )}
