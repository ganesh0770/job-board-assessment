...
export default function SeekerDashboard({ Job_see = [], token, API_URL = "http://localhost:8001" }: SeekerDashboardProps) {
  ...
  // const [activeJob, setActiveJob] = useState<Job_see | null>(null);
  // Track the ID instead of the whole object
const [activeJobId, setActiveJobId] = useState<number | null>(null);
  ...
  // Inject defaults to ensure detail pane state consistency
  const normalizedJobs = useMemo(() => {
    return Job_see.map(Job_see => ({
      ...Job_see,
      description: Job_see.description || fallbackDesc,
      postedDate: Job_see.postedDate || "2 days ago",
      requirements: Job_see.requirements || mockRequirements
    }));
  }, [Job_see]);

  // Compute and process searchable datasets
  const filteredJobs = useMemo(() => {
    ...
    if (sortBy === 'salary-high') {
      result.sort((a, b) => parseInt(b.salary.replace(/[^0-9]/g, '')) - parseInt(a.salary.replace(/[^0-9]/g, '')));
    } else if (sortBy === 'salary-low') {
      result.sort((a, b) => parseInt(a.salary.replace(/[^0-9]/g, '')) - parseInt(b.salary.replace(/[^0-9]/g, '')));
    }
    return result;
  }, [normalizedJobs, search, selectedType, sortBy]);

  // Automatically update selected detailed panel index context references
  // useEffect(() => {
  ...
  //     setActiveJob(null);
  //   }
  // }, [filteredJobs, currentView]);
  // Derive the active job dynamically during render
const activeJob = useMemo(() => {
  if (filteredJobs.length === 0) return null;
  
  // Try to find the previously selected job in the new filtered list
  const currentSelection = filteredJobs.find(j => j.id === activeJobId);
  
  // Fallback to the first job in the list if the previous selection is gone
  ...
  const handleApplySubmit = async (e: React.FormEvent) => {
    ...
    const id = toast.loading("Routing application vectors...");

    try {
      await new Promise(r => setTimeout(r, 700)); // Process payload transition latency
      toast.success("Application successfully compiled & sent!", { id });
      setAppliedJobIds(p => [...p, activeJob.id]);
      setIsModalOpen(false);
      setCoverLetter("");
    } catch {
      toast.error("Application dropped.", { id });
    } finally {
          ...
          <div className="h-full flex flex-col gap-4">
            ...
            <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-4 h-[calc(100%-5rem)] min-h-0">
              ...
              <div className="lg:col-span-5 overflow-y-auto space-y-2 pr-1 h-full max-h-full">
                {viewFilteredJobs.map(Job_see => {
                  const isCurrent = activeJob?.id === Job_see.id;
                  const isSaved = savedJobIds.includes(Job_see.id);
                  const isApplied = appliedJobIds.includes(Job_see.id);

                  return (
                    <div
                      key={Job_see.id}
                      // onClick={() => setActiveJob(Job_see)}
                      // Wherever you click a card to select it, change it to update the ID:
onClick={() => setActiveJobId(Job_see.id)}
                    ...
                    >
                      <div className="space-y-1">
                        <div className="flex items-center justify-between gap-2">
                          <span className="text-[10px] font-bold text-indigo-500 dark:text-indigo-400 uppercase tracking-wide">{Job_see.type}</span>
                          <span className="text-[10px] text-slate-400 font-mono">{Job_see.postedDate}</span>
                        </div>
                        <h4 className="font-bold text-sm text-slate-900 dark:text-white truncate">{Job_see.title}</h4>
                        <p className="text-xs text-slate-500 dark:text-slate-400">{Job_see.company} • {Job_see.location}</p>
                      </div>

                      <div className="flex items-center justify-between border-t border-slate-100 dark:border-slate-800/80 pt-2 text-xs">
                        <span className="text-emerald-600 dark:text-emerald-400 font-bold flex items-center gap-0.5"><DollarSign className="h-3 w-3"/>{Job_see.salary}</span>
                        <div className="flex items-center gap-1.5" onClick={e => e.stopPropagation()}>
                          <button onClick={() => toggleSaveJob(Job_see.id)} className={`p-1.5 rounded-lg border ${isSaved ? 'text-amber-500 border-amber-500/20 bg-amber-500/5' : 'text-slate-400 border-slate-200 dark:border-slate-800'}`}><Bookmark className={`h-3.5 w-3.5 ${isSaved?'fill-current':''}`}/></button>
                        ...
                        <div className="space-y-1.5 pt-2">
                          ...
                          <ul className="list-disc list-inside space-y-1 text-slate-600 dark:text-slate-300 font-medium">
                            {activeJob.requirements?.map((req, idx) => (
                              <li key={idx} className="list-item">{req}</li>
                            ))}
                          </ul>
                        </div>
                      </div>
          ...
          <div className="w-full max-w-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl p-6 relative flex flex-col gap-4 animate-in zoom-in-95 duration-150 text-slate-900 dark:text-slate-100" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleApplySubmit} className="space-y-4">
              ...
              <div className="flex items-center justify-end gap-2 text-xs font-bold">
                ...
                <button type="submit" disabled={isSubmitting} className="px-4.5 py-2 bg-indigo-600 hover:bg-indigo-500 rounded-xl text-white flex items-center gap-1.5 shadow-md shadow-indigo-600/15 disabled:opacity-50">{isSubmitting ? "Processing..." : "Dispatched App"} <Send className="h-3 w-3"/></button>
              </div>
            </form>
          </div>
        </div>
