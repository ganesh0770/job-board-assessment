...
//   onPostJobSubmit: (newJobData: any) => void;
//   onUpdateStatus: (appId: string, status: 'Shortlisted' | 'Rejected') => void;
// }

// export default function RecruiterDashboard({ jobsCount, applications, onPostJobSubmit, onUpdateStatus }: RecruiterDashboardProps) {
...
//     description: '',
//     requirements: '',
//     shiftType: 'Night' as 'Day' | 'Night' | 'Flexible',
...
//     e.preventDefault();
//     onPostJobSubmit(newJob);
//     setIsPostJobModalOpen(false);
//     setNewJob({ title: '', company: 'Globalco Ltd', description: '', requirements: '', shiftType: 'Night', location: 'Hitech City, Hyderabad (Onsite)', salary: '' });
//   };
...
//                       app.status === 'Shortlisted' ? 'bg-emerald-500/10 border border-emerald-500/20 text-emerald-400' :
...
//                     <button onClick={() => onUpdateStatus(app.id, 'Rejected')} className="px-2.5 py-1 border border-rose-500/20 bg-rose-500/5 hover:bg-rose-500/10 text-rose-400 text-[11px] font-bold rounded-lg transition">Reject</button>
//                     <button onClick={() => onUpdateStatus(app.id, 'Shortlisted')} className="px-2.5 py-1 bg-emerald-600 hover:bg-emerald-500 text-white text-[11px] font-bold rounded-lg transition flex items-center gap-1"><CheckCircle className="h-3 w-3" /> Shortlist</button>
//                   </div>
//                 )}
//               </div>
//             ))}
...
//                   <input type="text" required placeholder="e.g., ₹12,00,000 Lpa" className="w-full bg-slate-950 border border-white/10 focus:border-indigo-500 outline-none p-2.5 rounded-xl text-xs text-white" value={newJob.salary} onChange={e => setNewJob({...newJob, salary: e.target.value})} />
//                 </div>
//               </div>
//               <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
//                 <div>
//                   <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Shift Configuration</label>
//                   <select className="w-full bg-slate-950 border border-white/10 p-2.5 rounded-xl text-xs text-slate-300 font-semibold cursor-pointer" value={newJob.shiftType} onChange={e => setNewJob({...newJob, shiftType: e.target.value as any})}>
//                     <option value="Night">Night Shift (Required)</option>
...
//                 <input type="text" placeholder="React, Python, FastAPI" className="w-full bg-slate-950 border border-white/10 p-2.5 rounded-xl text-xs text-white" value={newJob.requirements} onChange={e => setNewJob({...newJob, requirements: e.target.value})} />
//               </div>
//               <div>
//                 <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Operational Description</label>
//                 <textarea required rows={3} placeholder="Delineate day-to-day operations parameters..." className="w-full bg-slate-950 border border-white/10 p-2.5 rounded-xl text-xs text-white resize-none" value={newJob.description} onChange={e => setNewJob({...newJob, description: e.target.value})} />
//               </div>
